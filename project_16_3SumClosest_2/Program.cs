...
static int ThreeSumClosest(int[] nums, int target)
{
    ...
    for (int i = 0; i < nums.Length - 1; i++)
    {
        ...
        {
            var rangeResult = positive ? result > 0 ? target - result : target + result : target - result;
            rangeResult = rangeResult > 0 ? rangeResult : 0 - rangeResult;

            var rangetemporaryResult = positive ? temporaryResult > 0 ? target - temporaryResult : target + temporaryResult : target - temporaryResult;
            rangetemporaryResult = rangetemporaryResult > 0 ? rangetemporaryResult : 0 - rangetemporaryResult;

            if (positive && result < target)
            {
                if (rangeResult >= rangetemporaryResult)
                {
                    result = temporaryResult;
                }
                leftIndex++;
            }
            else if (!positive && result > target)
            {
                if (rangeResult >= rangetemporaryResult)
                {
                    result = temporaryResult;
